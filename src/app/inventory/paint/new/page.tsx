...
import { PaintBottleCreate } from '@/types/inventory';
import { createPaintBottle } from '@/services/inventory';
...
export default function NewPaintBottlePage() {
  const router = useRouter();
  const [formData, setFormData] = useState<PaintBottleCreate>({
    color: '',
    brand: '',
    volume_ml: 0,
    current_volume_ml: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : 'Failed to create paint bottle');
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleChange = (field: keyof PaintBottleCreate, value: string | number) => {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Add a new paint bottle to inventory
            </p>
          </div>
        </div>
        ...
        <Card>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
              ...
              >
                {isSubmitting ? 'Adding...' : 'Add Paint Bottle'}
              </Button>
            </div>
          </form>
        </Card>
